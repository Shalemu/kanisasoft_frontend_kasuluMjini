...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const role = user?.role?.toLowerCase().replace(/_/g, ' ').replace(/\s+/g, ' ').trim();

    if (!token || role !== 'mtunza hazina') {
      router.push('/login');
    }
  }, []);
  ...
  return (
    <>
      <Head>
        <title>{`${activeTab} | Mtunza Hazina`}</title>
      </Head>
          ...
          <div className="flex items-center justify-between px-6 py-5">
            <div className="flex items-center gap-3 font-bold text-2xl">
             <Image src="/KANISASOFT1.png" alt="FPCT Logo" width={200} height={200} />
              <span className="tracking-wide text-white">MTUNZA HAZINA</span>
            </div>
