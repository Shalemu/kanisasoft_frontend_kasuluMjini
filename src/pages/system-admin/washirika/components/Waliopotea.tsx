...
import { FaCheck, FaTimes, FaUserCheck, FaTrash } from 'react-icons/fa';
...
export default function Waliopotea() {
  ...
  useEffect(() => {
    async function fetchDetainedMembers() {
      const data = await apiFetch('/users');
      if (data?.users) {
        const lostMembers = data.users.filter(
          (u: any) => ['detained', 'lost', 'left', 'deceased'].includes(u.membership_status)
        );
        setMembers(lostMembers);
      }
    }

    fetchDetainedMembers();
  }, []);
  ...
  return (
    <div className="bg-white rounded shadow border border-gray-200 p-4 overflow-x-auto">
      ...
      {members.length === 0 ? (
        <div className="text-center py-16 text-gray-500">Hakuna waliopotea kwa sasa.</div>
      ) : (
        <>
          {/* Table Header */}
          ...
          {members.map((member) => (
            ...
            >
              ...
              <div className="md:col-span-2 text-yellow-600 font-bold flex gap-2 items-center capitalize">
                <FaTimes /> {statusLabels[member.membership_status || ''] || 'Haijulikani'}
              </div>
            </div>
          ))}
        </>
