...
export default function KalendaTab() {
  ...
  const getEvents = (date: Date) => {
    ...
    return events.filter(
      (e) => e.date === dateStr && (selectedCategory === '' || e.category === selectedCategory)
    );
  };
    ...
    for (let i = 0; i < 6; i++) {
      ...
      for (let j = 0; j < 7; j++) {
        if ((i === 0 && j < startDay) || day > totalDays) {
          ...
          const todayEvents = getEvents(date);
            ...
            <td key={j} className="border border-gray-300/40 h-28 align-top p-1 bg-white text-xs">
              ...
              <ul className="space-y-1 overflow-y-auto max-h-20 pr-1">
                {todayEvents.map((event, i) => (
                  <li key={i} className="bg-blue-100 px-2 py-1 rounded text-blue-900 text-xs">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        ...
        <div className="min-w-[200px]">
          <select
            ...
            className="w-full border border-gray-300 rounded px-3 py-2 text-sm shadow-sm bg-white focus:ring-2 focus:ring-blue-300"
          >
